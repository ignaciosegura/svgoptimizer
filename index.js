...
class SVGOptimizer {
  constructor (input = 'input', output = 'output', settingsFile = 'svg-optimize-default.json') {
    let cmdArgs = this.getCmdArguments();
    this.config = {
      inputFolder: cmdArgs.input || input,
      outputFolder: cmdArgs.output || output,
      settingsFile: cmdArgs.settings || settingsFile
    };
    this.SVGOSettings = JSON.parse(fs.readFileSync(this.config.settingsFile, 'utf8'));
